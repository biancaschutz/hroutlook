 ...
 summarize(
  ...
  ungroup()
    ...
    ggplot() +
     ...
     geom_text(data = lifeexp_avgs, aes(x = region_lifeexp, y = as.numeric(as.factor(name)) + .35, label = paste0("Regional average: ", round(region_lifeexp, 1), " years")), hjust = 1) + 
    geom_beeswarm(data = lifeexp, aes(x = Value, y = as.numeric(as.factor(name)), color = name), cex = 2) +
